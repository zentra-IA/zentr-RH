...
const FEATURE_LABELS: Record<string, string> = {
  ...
  radar: "Radar de Candidatos",
  campanhas_vagas: "Campanhas de Vagas",
  inbox: "Caixa de Entrada",
  chatbot_ia: "Mensagens / Chatbot",
  whatsapp: "WhatsApp",
  entrevistas: "Entrevistas",
  contratacoes: "Contratações",
  bi_recrutamento: "BI de Recrutamento",
  importacao_curriculos: "Importação de Currículos",
  matching_ia: "Matching com IA",
};
    ...
    <main style={styles.page}>
      <div style={styles.container}>
        <section style={styles.hero}>
          <p style={styles.kicker}>Zentra RH Master</p>
          <h1 style={styles.heroTitle}>Empresas e Usuários</h1>
          <p style={styles.heroText}>
            Crie empresas, gerencie usuários, controle planos, permissões e recursos do Zentra RH.
          </p>
        </section>
                    ...
                    {(usersData?.users || []).map((user: any) => (
                      <div key={user.id} style={styles.userCard}>
                        ...
                        <div style={styles.grid2}>
                          ...
                          <select style={{ ...styles.input, gridColumn: "1 / -1" }} value={user.role || "recrutador"} onChange={(e) => setUsersData((prev: any) => ({ ...prev, users: prev.users.map((u: any) => u.id === user.id ? { ...u, role: e.target.value } : u) }))}>
                            {ROLES.map((role) => (
                              <option key={role} value={role}>{role}</option>
                            ))}
                          </select>
