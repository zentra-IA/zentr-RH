...
function detectRhIntent(message: string) {
...
function statusFromIntent(intent: string) {
  const map: Record<string, string> = {
    ...
    AGENDOU_ENTREVISTA: "entrevista_agendada",
    QUER_ENTREVISTA: "quer_agendar_entrevista",
    RESPONDEU: "respondeu",
  };
...
const LEGACY_KANBAN_STATUS_MAP: Record<string, string> = {
  ...
  entrevista: "quer_agendar_entrevista",
  agendamento: "quer_agendar_entrevista",
...
function shouldForceInterviewStatus(message: string, intent: string, reply?: string | null) {
  const text = normalizeText(message);
  const normalizedIntent = String(intent || "").toUpperCase();

  if (normalizedIntent === "QUER_ENTREVISTA") return true;
  ...
  if (
    responseText.includes("agenda/") ||
    responseText.includes("link agendamento") ||
    responseText.includes("horarios disponiveis") ||
    responseText.includes("horários disponíveis")
  ) {
    return true;
  }
...
function applyVariables(text: string, lead: any, extra: any = {}) {
  ...
  const scheduleLink = extra?.scheduleLink || extra?.linkAgendamento || "";
  const companyName = extra?.companyName || extra?.company || "Zentra RH";

  return String(text || "")
    .replaceAll("{nome}", lead?.name || extra?.name || "Candidato")
    ...
    .replaceAll("{data}", extra?.interviewDate || "")
    .replaceAll("{horario}", extra?.interviewDate || "")
    .replaceAll("{data_entrevista}", extra?.interviewDate || "")
...
function intentAliases(intent: string) {
  ...
  const aliases: Record<string, string[]> = {
    RESPONDEU: ["DEFAULT", "OPENING", "RH_ABERTURA", "RESPONDEU"],
    QUER_ENTREVISTA: [
      "INTERESSE_ENTREVISTA",
      "RH_ENTREVISTA",
      "AGENDAMENTO",
      "MENSAGEM_AGENDAMENTO",
      "QUER_ENTREVISTA",
    ],
    AGENDOU_ENTREVISTA: ["AGENDAMENTO", "RH_RELEMBRETE", "CONFIRMACAO_ENTREVISTA"],
    SEM_INTERESSE: ["SEM_INTERESSE"],
  };
...
async function findRhTriggeredTemplate({
  supabase,
  message,
  lead,
  companyId,
...
async function getFinalRhReply({
  supabase,
  intent,
  message,
  lead,
...
export async function POST(req: Request) {
  try {
    ...
    */
    ...
    const intent = detectRhIntent(message);
  ...
  if (!lead) {
    const created = await supabase
      ...
      .insert({
        company_id: companyId,
        branch_id: branchId,
        name: pushName || "Candidato WhatsApp",
...
if (queueContext) {
  ...
  if (Object.keys(leadJobPatch).length) {
    ...
    if (updateLeadJobError) {
      console.error("ERRO AO VINCULAR LEAD AO LOTE/VAGA:", updateLeadJobError);
    } else if (updatedLead) {
      lead = updatedLead;
    }
  }
...
console.log("CONTEXTO FINAL DO LEAD PARA TEMPLATE:", {
  ...
  batch_id: lead.batch_id,
    ...
    const finalReply = await getFinalRhReply({
      supabase,
      intent,
      message,
      lead,
    ...
    const finalKanbanStatus =
      ...
      (shouldForceInterviewStatus(message, intent, finalReply.reply)
        ? "quer_agendar_entrevista"
        : null);
    ...
    return NextResponse.json({
      success: true,
      action: replied ? "resposta_template_rh" : "kanban_atualizado",
      intent,
    ...
    console.error("ERRO API WHATSAPP INCOMING RH:", error);
